...
- **Nix Flake Monorepo**: Manages NixOS (`desktop`, `surface`, `wsl`) and nix-darwin (`mac`).
- **OS Entrypoints**: `flake.nix` exports `nixosConfigurations` and `darwinConfigurations`. Host-specific configurations live in `hosts/<name>/default.nix` and `hosts/<name>/configuration.nix`, which typically import the shared root `configuration.nix` (except mac/wsl where appropriate).
- **Home Manager**: Wired directly into the NixOS/darwin module tree via `home-manager.nixosModules.home-manager` (`useGlobalPkgs = true`). HM configurations are under `home/` with per-host entrypoints (`home/mac.nix`, `home/wsl.nix`, `home/surface.nix`, and `home/default.nix` for desktop). **Do not use standalone `home-manager switch`.**
...
- **macOS Rebuild**: `nh darwin switch` (or fallback to `darwin-rebuild switch --flake .#mac`).
